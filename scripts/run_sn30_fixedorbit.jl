#!/usr/bin/env julia
# Binary supernova on a FIXED Keplerian circular orbit.
#
# Variant of run_sn30_sinks.jl: BH trajectories are prescribed analytically by
# KeplerOrbit / kepler_update! instead of being integrated by nbody_step!.
# Accretion (`accrete!`) is still enabled, so M_BH(t) grows with fallback;
# that growth does NOT back-react on the orbit (Ω is frozen at t=0).
#
# Test goal: isolate the hydrodynamic response (sink accretion, wake structure,
# torques on gas) from orbital dynamics.  Direct comparison to run_sn30_sinks.jl
# reveals how much of the post-SN orbital evolution is driven by gas forces
# versus BH-BH gravity + natal kick.
#
# Caveat: on a fixed orbit, energy and momentum are not globally conserved —
# the orbit kinematically overrides any dynamical feedback.  Diagnostics that
# track E_BH_kin, momentum, etc. will drift; this is by design.
#
# Usage:
#   julia --project=. scripts/run_sn30_fixedorbit.jl [--gpu] [--nx NX]
#                                                   [--torque-free]
#
# Output written to demo1/output_sn30_fixedorbit/:
...
outdir_arg = let v = "demo1/output_sn30_fixedorbit"
    for (i, arg) in enumerate(ARGS)
...
# ---------------------------------------------------------------------------
# Physical parameters — identical to run_sn30_sinks.jl
...
const E_SN       = 0.3
const R_BOMB     = R_STAR
const V_KICK     = [0.0, 0.0, 0.0]   # kick is silently ignored — fixed orbit
const F_SINK     = 1.0
const RHO_FLOOR  = rho_floor_arg
const P_FLOOR    = 1e-6
# Skip cells at or below 2× the density floor — prevents BHs from draining the
# ambient "vacuum" (floor × domain volume ≈ 0.5 units of gas at ρ_floor=1e-3).
const RHO_SINK_MIN = 2.0 * RHO_FLOOR
const CFL        = 0.4
...
# ---------------------------------------------------------------------------
# CoM recentring (identical to sinks/nosink runs)
...
# ---------------------------------------------------------------------------
# Step 3: thermal bomb + BH2 activation
...
bh2 = BlackHole(
    ...
    [0.0, v_bh2_y, 0.0],        # natal kick ignored on fixed orbit
    M_BH2_INIT,
    eps_bh,
    units.c_code,
    r_floor)
...
# ---------------------------------------------------------------------------
# Fixed Kepler orbit — captured at t=0 from the current two-BH state.
# Ω is frozen; mass growth via accrete! does not change the orbital radius.

orbit = KeplerOrbit(bhs)
P_orb = 2π / orbit.Ω
@info "Fixed Kepler orbit" Ω=orbit.Ω P_orb=P_orb nhat=orbit.nhat com=orbit.com
...
# ---------------------------------------------------------------------------
# Coupled SSP-RK3 step with fixed orbit + sinks + accretion.
# BHs are advanced kinematically to t_new = t + dt; F_gas is computed for the
# diagnostic record but does not drive BH motion.

function coupled_step_fixedorbit!(U, Un, dU, Fx, Fy, Fz, bhs, orbit, t_new, dt,
                                   nx, ny, nz, dx, x0, y0, z0, γ,
                                   f_sink, ρ_floor, P_floor, torque_free)
    # Diagnostic-only gas→BH force (not fed back into motion).
    ...
    # Prescribe BH positions and velocities at end-of-step time.
    kepler_update!(bhs, orbit, t_new)
    ...
    # Accrete mass/momentum into BHs; the pos/vel override at the next step
    # discards the velocity change while keeping the mass change.
    for bh in bhs
...
@info "Starting fixed-orbit evolution" t_end=T_END P_orb=round(P_orb,digits=3) torque_free=torque_free
t_wall = time()

function run_evolution!(U, Un, dU, Fx, Fy, Fz, bhs, orbit,
    ...
    while t < T_END - 1e-12
        ...
        t_new = t + dt
        F_gas = coupled_step_fixedorbit!(U, Un, dU, Fx, Fy, Fz, bhs, orbit,
                                          t_new, dt,
...
t_final, n_steps = run_evolution!(U, Un, dU, Fx, Fy, Fz, bhs, orbit,
                                   traj_file, diag_file, snap_path, torque_free)
...
@info "Orbit invariants" r_sep=round(sqrt(sum((bhs[2].pos .- bhs[1].pos).^2)),digits=6) Ω=orbit.Ω
